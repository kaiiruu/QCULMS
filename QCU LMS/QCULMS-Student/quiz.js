...
// ── FILL QUIZ META WITH LIVE DATE/TIME ──
function fillMeta() {
  const now = new Date();
  const days = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
  const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  ...
  document.getElementById('quiz-meta-1').innerHTML = metaText;
  const m2 = document.getElementById('quiz-meta-2'); if (m2) m2.innerHTML = metaText;
}
fillMeta();
